...
void ModelPart::loadSTL(const QString& fileName ) {
    ...
    // Create and configure STL reader
    ...
    // Mapper connects geometry (polydata) to the rendering pipeline
    mapper = vtkSmartPointer<vtkPolyDataMapper>::New();
    mapper->SetInputConnection(stlReader->GetOutputPort());
    
    /* 3. Initialise the part's vtkActor and link to the mapper */
    // Actor represents an object in the scene
...
{
    // Return the raw pointer managed by vtkSmartPointer.
    // This avoids relying on implicit conversions and makes the intent clear.
